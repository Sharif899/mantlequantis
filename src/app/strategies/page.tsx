...
import { Card, Badge, Button, SectionTitle, EmptyState } from "@/components/ui";
...
export default function StrategiesPage() {
  ...
  async function handleDeploy() {
    ...
    try {
      // ── Step 1: Register agent identity on Mantle Sepolia (ERC-8004 inspired) ──
      ...
      // ── Step 3: Start agent simulation loop ───────────────────────────────────
  ...
  function startAgentSimulation(strategyId: string) {
    const agentInterval = setInterval(() => {
      ...
      store.agentTick(agent.id, pnlDelta, agent.pair, 0.94);
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          {registerTxHash && (
            <a
              href={`${MANTLE_EXPLORER}/tx/${registerTxHash}`}
              target="_blank"
      ...
      {!TRADE_LOGGER_ADDRESS && (
        <div className="mb-5 px-4 py-3 rounded-xl border border-amber-800 bg-[#1A1A0D] text-amber-400 text-sm flex items-center gap-3">
          <span>⚠️</span>
          <span>
            Deploy <code className="bg-black/20 px-1 rounded">MantleQuantTradeLogger.sol</code> to Mantle Sepolia
            for on-chain agent registration. See README for instructions.{" "}
            <a href="https://faucet.sepolia.mantle.xyz" target="_blank" rel="noopener noreferrer" className="underline">
              Get testnet MNT →
            </a>
          </span>
        </div>
        ...
        {STRATEGIES.map((s) => {
          ...
          return (
            <div
              ...
              className={clsx(
                "bg-mantle-card border rounded-xl p-5 cursor-pointer transition-all",
                isSelected
                  ? "border-mantle-purple shadow-[0_0_20px_rgba(127,119,221,0.2)]"
                  : "border-mantle-border hover:border-mantle-border/80"
              )}
            >
              ...
              {activeAgent?.erc8004TokenId && (
                <div className="mt-3 pt-3 border-t border-mantle-border">
                  <a
                    href={`${MANTLE_EXPLORER}/tx/${activeAgent.erc8004TokenId}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={(e) => e.stopPropagation()}
                    className="text-[10px] text-mantle-purple hover:underline font-mono"
                  >
        ...
        <Card className="mb-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="font-semibold text-white">
                {selected.icon} Configure: {selected.name}
              </div>
              <div className="text-xs text-mantle-muted mt-1">
                Cash available: ${portfolio.cash.toLocaleString(undefined, { maximumFractionDigits: 2 })} ·
                {TRADE_LOGGER_ADDRESS
                  ? " Agent will be registered on Mantle Sepolia"
                  : " Deploy contract to enable on-chain registration"}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-5">
            {selected.params.map((p) => (
              <div key={p.key}>
                <label className="text-xs text-mantle-muted mb-1.5 block">
                  {p.label} {p.unit && <span className="opacity-60">({p.unit})</span>}
                </label>
                <input
                  type="number"
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <Button
              variant="outline"
              onClick={() => window.open("/transparency", "_self")}
            >
              🔍 View Strategy Logic
            </Button>
            {!isConnected && (
              <p className="text-xs text-mantle-muted self-center">Connect wallet to deploy</p>
            )}
          </div>
        </Card>
      )}
      ...
      {backtestResult && (
        <Card className="mb-5">
            ...
            {[
              ...
              <div key={m.label} className="bg-[#0D0D14] border border-mantle-border rounded-xl p-3">
                ...
                <div className={clsx(
                  ...
                  m.pos && "text-mantle-teal",
                  m.neg && "text-mantle-coral"
                )}>
                  {m.value}
                </div>
              </div>
          ...
          <p className="text-xs text-mantle-muted">
            ⚠️ Paper trading only. Past simulated performance does not guarantee future results.
          </p>
        </Card>
      )}
                ...
                <div key={agent.id} className="flex items-center justify-between py-3 border-b border-mantle-border last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-mantle-muted">
                        {agent.trades} trades · Started {new Date(agent.startedAt).toLocaleTimeString()}
                      </div>
                      {agent.erc8004TokenId && (
                        <a
                          href={`${MANTLE_EXPLORER}/tx/${agent.erc8004TokenId}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-[10px] text-mantle-purple hover:underline font-mono"
                        >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className={clsx("text-sm font-semibold num", agent.pnl >= 0 ? "text-mantle-teal" : "text-mantle-coral")}>
                        {agent.pnl >= 0 ? "+" : ""}${agent.pnl.toFixed(2)}
                      </div>
                      <Badge variant={agent.active ? "purple" : "gray"} dot={agent.active} className="text-[10px]">
